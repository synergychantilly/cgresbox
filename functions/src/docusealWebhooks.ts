...
export const docusealWebhook = onRequest(
  {
    cors: true
  },
  async (request, response) => {
    try {
      ...
      const signature = request.headers['x-docuseal-signature'] as string;
      if (signature) {
        logger.info('Webhook signature received but verification is disabled for now');
      }

      const payload: DocuSealWebhookPayload = request.body;
      ...
      logger.error('Error processing DocuSeal webhook', error);
...
async function processWebhookEvent(payload: DocuSealWebhookPayload) {
  try {
    // Extract email directly from payload data
    const userEmail = payload.data.email;
    if (!userEmail) {
      logger.warn('No email found in webhook payload', { submissionId: payload.data.id });
      return;
    }
    ...
    if (!templatesQuery.empty) {
      ...
      logger.info('Found template by DocuSeal template ID', {
        docusealTemplateId,
        documentTemplateId,
      ...
      if (!nameQuery.empty) {
        ...
        logger.info('Found template by name (fallback)', {
          templateName: payload.data.template.name,
          documentTemplateId
        });
      }
    ...
    if (!documentTemplateId) {
      logger.warn('Document template not found', { 
        docusealTemplateId,
        templateName: payload.data.template.name,
        submissionId: payload.data.id,
        userEmail 
    ...
    switch (payload.event_type) {
      case 'form.viewed':
        ...
        logger.info('Processing form.viewed event', { submissionId: payload.data.id, email: payload.data.email });
        break;
      ...
      case 'form.started':
        ...
        logger.info('Processing form.started event', { submissionId: payload.data.id, email: payload.data.email });
        break;

      case 'submission.created':
        updateData.status = 'started';
        updateData.startedAt = new Date(payload.data.created_at);
        logger.info('Processing submission.created event', { submissionId: payload.data.id, email: payload.data.email });
        break;
      ...
      case 'form.opened':
        ...
        logger.info('Processing form.opened event', { submissionId: payload.data.id, email: payload.data.email });
        break;
...
export const cleanupWebhookEvents = onRequest(async (request, response) => {
  try {
...
export const initializeUserDocuments = onRequest(async (request, response) => {
  try {
    if (request.method !== 'POST') {
      response.status(405).send('Method Not Allowed');
      return;
    ...
    logger.error('Error initializing user documents', error);
    response.status(500).json({
      success: false,
